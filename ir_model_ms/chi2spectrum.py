...
FluxMeasurement = namedtuple("FluxMeasurement", ["line", "wavelength", "lam_min", "lam_max", "flux", "flux_error"])


class Chi2Spectrum:
    _Chi2Comparison = namedtuple("Chi2Comparison", ["line", "wavelength", "lam_min", "lam_max",
                                                    "flux", "flux_error", "flux_model", "chi2"])
    ...
    def load_file(self, fname, form=None): # Form can be either 's' or 'b'
        ...
        form: 's' or 'b'
            If s, fname must be a pd.DataFrame obj in the Line Protocol v2.1 format.
            if b, fname must be txt file (original bruderer method).
        ...
        """
        # Giving the option to use .xlsx or pd.DataFrame object input
        """
        df = fname
        if type(df) == pd.core.frame.DataFrame:
            ...
            raise ValueError("type() is neither pd.DataFrame nor str('...xlsx')")
        """
        # Maintaining columns, inserting empty arrays as placeholder

        if form == 'original':
            lam_min, lam_max, flux, flux_error = np.loadtxt(fname, unpack=True)
            line = np.arange(len(flux))
            wavelength = np.arange(len(flux))

        elif form == 's':
            df = fname
            #df.set_index(df['line'])  # Doesn't work? index is still 1,2,3,...
            line = df['line']
            wavelength = df['wavelength']  # wavelength
            lam_min = df['lammin']  # lammin
            lam_max = df['lammax']  # lammax
            flux = df['flux']  # flux
            flux_error = df['flux_error']  # flux_error

        for d in zip(line, wavelength, lam_min, lam_max, flux, flux_error):
    ...
    def get_table(self):
        ...
        return pd.DataFrame({'line':[c.line for c in self._chi2],
                             'wavelength': [c.wavelength for c in self._chi2],
                             'lam_min': [c.lam_min for c in self._chi2],
